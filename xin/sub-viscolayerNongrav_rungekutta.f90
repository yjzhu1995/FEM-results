...
contains
	...
	subroutine Spheroidalsol(lambda,mu,rho,N,rr,Yr)
!
...
!
	...
	Yr = dcmplx(0.d0, 0.d0)
	Yr(1,1) =  (NN*lambda+(NN-2.D0)*mu)/fl2*(NN+1.D0)
	Yr(1,2) =  dcmplx(NN, 0.d0)

	Yr(2,1) = dcmplx(1.d0, 0.d0)
	Yr(2,2) = dcmplx(1.d0, 0.d0)

	Yr(3,1) = 2.d0*mu*((NN**2-NN-3.D0)*lambda+(NN**2-NN-2.D0)*mu)/fl2*(NN+1.D0)
	Yr(3,2) = 2.d0*mu*NN*(NN-1.D0)

	Yr(4,1) = 2.d0*mu*((NN**2+2.D0*NN)*lambda+(NN**2+2.D0*NN-1.D0)*mu)/fl2
	Yr(4,2) = 2.d0*mu*(NN-1.D0)

	temp = 4.d0*pi*UGC*rho
	Yr(5,1) = -temp*mu*(NN+1.d0)/fl2
	Yr(5,3) = dcmplx(1.d0, 0.d0)

	Yr(6,1) = -temp*dble(NN+1)*(NN*lambda+(3.d0*NN+1.d0)*mu)/fl2
	Yr(6,2) = dcmplx(-temp*NN)
...
! --------------------------------------------------------
	...
	subroutine fundamatrixA(lambda,mu,xi2,N,rr,nm,Yr)
!
...
!
	...
	Yr(3,1) =  12.d0*kappa*mu/beta/rr/rr
	Yr(3,2) = -6.d0*NN*(NN+1.d0)*kappa*mu/beta/rr/rr
	Yr(3,3) = -4.d0*mu/beta/rr
	Yr(3,4) =  dcmplx(NN*(NN+1.d0)/rr)

	Yr(4,1) = -6.d0*kappa*mu/beta/rr/rr
...
!  propagate the 6x3 solution from r0 to r1 by using 4th-order Runge-Kutta (RK4) method
	...
	call fundamatrixA(lambda,mu,xi,N,r0,nl,f)
	k1 = matmul(f,y0)
	k1 = h*k1

	rt = r0+h/2.d0
	y1 = y0+k1/2.d0
	call fundamatrixA(lambda,mu,xi,N,rt,nl,f)
	k2 = matmul(f,y1)
	k2 = h*k2
	...
	y1 = y0+k3
	call fundamatrixA(lambda,mu,xi,N,r1,nl,f)
	k4 = matmul(f,y1)
	k4 = h*k4
...
!
	...
	do k=nlayer-1,1,-1
		...
		do while(r2 < rlayer(k,1))
			...
			call propagation(NN,nm,N,Yrs,lambda(k),mu(k),xi(k),r2,r1,Yrs1)
			call lufact(Yrs1)
			Yrs = Yrs1
			r2 = r1
		enddo
	...
	do while(r2 < rlayer(k,1))
		r1 = min(r2+step/Re,rlayer(k,1))
		call propagation(NN,nm,nm,Yrs2,lambda(k),mu(k),xi(k),r2,r1,Yr1)
! 		call lufact(Yr1)
