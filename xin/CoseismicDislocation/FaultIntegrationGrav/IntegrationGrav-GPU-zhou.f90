...
!  interpolation
	...
	green12x(:,:) = greenval(1,:,:,4)
	green12y(:,:) = greenval(1,:,:,5)
	green12z(:,:) = greenval(1,:,:,6)
	green32x(:,:) = greenval(2,:,:,4)
	green32y(:,:) = greenval(2,:,:,5)
	green32z(:,:) = greenval(2,:,:,6)
	green33x(:,:) = greenval(3,:,:,4)
	green33y(:,:) = greenval(3,:,:,5)
	green33z(:,:) = greenval(3,:,:,6)
	green220x(:,:) = greenval(4,:,:,4)
	green220y(:,:) = greenval(4,:,:,5)
	green220z(:,:) = greenval(4,:,:,6)
...
!  write output file
	open(unit=21, file="gravity.dat", iostat=ios, action="write")
	if ( ios /= 0 ) stop "Error opening file displacements"
	write(21,'(A)') "lon,  lat.  geoid(m),  gravity_fix(ugal),   gravity_defor(ugal)"
	do i=1,nobs
		write(21,33) obs(i)
	end do
	close(21)
...
!$acc routine(greatcircle,sineangle) seq
	...
	u12(1) = dsin(2.d0*z0)*u12x
	u32(1) = dsin(z0)*u32x
	u13(1) = dsin(z0+pi/2.d0)*u32x
	u33(1) = u33x
	u22(1) = u220x+dcos(2.d0*z0)*u12x
