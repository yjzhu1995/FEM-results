...
contains
	...
	subroutine sphericalbesselj(x,n,z)
! calculate j_n+1(x)/j_n(x), the result is very stable
		implicit none
		integer :: n, i
		complex*16 :: x, z
		...
		temp = x/dcmplx(2.d0*nmax+3.d0)
		do i=nmax,n,-1
			temp = x*x/(dcmplx(2.d0*i+3.d0)-temp)
		end do

		z = temp

	end subroutine sphericalbesselj

! --------------------------------------------------------

	subroutine Spheroidalsol(lambda,mu,rho,N,r,Yr)
!
...
!
		...
		complex*16 :: Yr(nl,nc)
		complex*16 :: xi, beta, vp2, vs2, X2, gamma2, Ch1, W, work1
		complex*16 :: k, x, y
		...
		vp2 = beta/rho
		vs2 = mu/rho
		xi = dcmplx(4.d0*pi*UGC*rho/3.d0)
		X2 = xi/vp2
		gamma2 = NN*(NN+1.d0)*xi*xi/vp2/vs2

		Yr = dcmplx(0.d0,0.d0)
! 		work1 = (X2+zsqrt(X2*X2+4*gamma2))*0.5d0
		work1 = X2*(1.d0+zsqrt(1.d0+4.d0*NN*(NN+1.d0)*beta/mu))*0.5d0
		k = zsqrt(work1)
		x = k*r
		call sphericalbesselj(x, N, y)
		Ch1 = -xi/work1/vs2
		W = -(1.d0+Ch1+Ch1*NN)
		Yr(1,1) = (W*NN+y)/work1/r
		Yr(2,1) = (W+Ch1*y)/work1/r
		Yr(3,1) = 2.d0*NN*(NN-1.d0)*W*mu/work1/r/r + beta &
		         + 2.d0*(Ch1*NN*(NN+1.d0)-2.d0)*mu*y/work1/r/r
		Yr(4,1) = 2.d0*(NN-1.d0)*W*mu/work1/r/r + Ch1*mu - 2.d0*(Ch1-1.d0)*y*mu/work1/r/r
		Yr(5,1) = -3.d0*xi/work1
		Yr(6,1) = 3.d0*(NN+1.d0)*(Ch1*NN-1.d0)*xi/work1/r
...
! 		work1 = (X2-zsqrt(X2*X2+4*gamma2))*0.5d0
		...
		Ch1 = -xi/work1/vs2
		W = -(1.d0+Ch1+Ch1*NN)
		Yr(1,2) = (W*NN+y)/work1/r
		Yr(2,2) = (W+Ch1*y)/work1/r
		Yr(3,2) = 2.d0*NN*(NN-1.d0)*W*mu/work1/r/r + beta &
		         + 2.d0*(Ch1*NN*(NN+1.d0)-2.d0)*mu*y/work1/r/r
		Yr(4,2) = 2.d0*(NN-1.d0)*W*mu/work1/r/r + Ch1*mu - 2.d0*(Ch1-1.d0)*y*mu/work1/r/r
		Yr(5,2) = -3.d0*xi/work1
		Yr(6,2) = 3.d0*(NN+1.d0)*(Ch1*NN-1.d0)*xi/work1/r


		Yr(1,3) = dcmplx(NN*r)
		Yr(2,3) = dcmplx(r)
		Yr(3,3) = 2.d0*mu*NN*(NN-1.d0)
		Yr(4,3) = 2.d0*mu*(NN-1.d0)
		Yr(5,3) = NN*xi*r*r
		Yr(6,3) = 2.d0*NN*(NN-1.d0)*xi*r
...
!
!   Dimensionless spheroidal fundamental matrix, a 4x4 matrix consisting of ODE's equation.
...
!
	...
	Yr(3,1) =  12.d0*kappa*mu/beta/rr/rr-4.d0*Cg/rr
	Yr(3,2) =  NN*(NN+1.d0)*(Cg/rr-6.d0*kappa*mu/beta/rr/rr)
	Yr(3,3) = -4.d0*mu/beta/rr
	Yr(3,4) =  dcmplx(NN*(NN+1.d0)/rr)
	Yr(3,5) =  (NN+1.d0)*Cg/rr
	Yr(3,6) =  -Cg

	Yr(4,1) =  Cg/rr-6.d0*kappa*mu/beta/rr/rr
	Yr(4,2) =  2.d0*mu*((2.d0*NN*(NN+1.d0)-1.d0)*lambda+2.d0*(NN*(NN+1.d0)-1.d0)*mu)/beta/rr/rr
...
! 
	...
	real*8 :: dept
	complex*16 :: LNT(ns),d(nc,ns)
! 	complex*16 :: X(N,ns),C(nm,ns)
...
! 	call COEFFT(nlayer,rlayer,mu,NN,dept,X)
	call COEFFT(nlayer,rlayer,mu,NN,dept,d)
...
! 			Yrs2 = Yr1/maxval(zabs(Yr1))
	...
	do k=1,ns
		d(1,k) = -Yrs2(2,k)/Yrs(2,1)*Yrs(1,1) + Yrs2(1,k) 
	end do
