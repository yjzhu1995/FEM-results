...
program Integ
	...
	integer :: ndepth, nepdis, ngreenls, ntime
...
!  *****
	...
	allocate(greenval(ntime,4,ndepth,nepdis,6))
...
!  read green's function
	do k=1,ndepth
		...
		do i=1,nepdis
			read(20,*) temp,(greenval(j,1,k,i,1),greenval(j,1,k,i,2),greenval(j,1,k,i,3), &
				             greenval(j,1,k,i,4),greenval(j,1,k,i,5),greenval(j,1,k,i,6), &
			                 greenval(j,2,k,i,1),greenval(j,2,k,i,2),greenval(j,2,k,i,3), &
			                 greenval(j,2,k,i,4),greenval(j,2,k,i,5),greenval(j,2,k,i,6), &
			                 greenval(j,4,k,i,1),greenval(j,4,k,i,2),greenval(j,4,k,i,3), &
			                 greenval(j,4,k,i,4),greenval(j,4,k,i,5),greenval(j,4,k,i,6), &
			                 greenval(j,3,k,i,1),greenval(j,3,k,i,2),greenval(j,3,k,i,3), &
			                 greenval(j,3,k,i,4),greenval(j,3,k,i,5),greenval(j,3,k,i,6),j=1,ntime)
		end do
		close(20)
	end do
...
!  interpolation
	...
	green12x(:,:) = greenval(n,1,:,:,1)
	green12y(:,:) = greenval(n,1,:,:,2)
	green12z(:,:) = greenval(n,1,:,:,3)
	green32x(:,:) = greenval(n,2,:,:,1)
	green32y(:,:) = greenval(n,2,:,:,2)
	green32z(:,:) = greenval(n,2,:,:,3)
	green33x(:,:) = greenval(n,3,:,:,1)
	green33y(:,:) = greenval(n,3,:,:,2)
	green33z(:,:) = greenval(n,3,:,:,3)
	green220x(:,:) = greenval(n,4,:,:,1)
	green220y(:,:) = greenval(n,4,:,:,2)
	green220z(:,:) = greenval(n,4,:,:,3)
...
!  write output file
	...
	write(21,'(A)') "lon,  lat.  Ux(m),  Uy(m),   Uz(m) ..."
	do i=1,nobs
		write(21,33) obs(i), (uxyz(k,i),k=1,ntime)
	end do
	close(21)
33  format(2f10.5,100E20.10)
